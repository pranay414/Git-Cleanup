...
module.exports = {
    ...
    public: async (owner_name, repo_details, token) => {
        ...
        for (repo of repo_details) {
            ...
            await axios.patch(`${GITHUB_API_BASE_URL}` + repo_url + repo.name + `?access_token=${token}`, { name: "AndrewNg-ML-Coursera", private: "false" } )
            console.log(chalk.green(`Successfully made repository: ${repo.name} public!`))
        }
    },
